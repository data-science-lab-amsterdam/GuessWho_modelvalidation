...
def bulma_dropdown(id, options):
    return html.Div(className='select', children=[
...
app.layout = html.Div([
    ...
    html.Div(className='columns', children=[
        html.Div(className='column', children=[
            bulma_field(label='Haarkleur', component=dcc.Dropdown(id='input-hair_colour', 
                options=[{'label': 'licht', 'value': 'light'},
                        {'label': 'Donker', 'value': 'dark'}])),

            bulma_field(label='Haartype', component=dcc.Dropdown(id='input-hair_type', 
                options=[{'label': 'krullend', 'value': 'curly'},
                {'label': 'stijl', 'value': 'straight'}])),

            bulma_field(label='Haarlengte', component=dcc.Dropdown(id='input-hair_length', 
                options=[{'label': 'kaal', 'value': 'bold'},
                {'label': 'kort', 'value': 'short'},
                {'label': 'lang', 'value': 'long'}])),

            bulma_field(label='Geslacht', component=dcc.Dropdown(id='input-gender', 
                options=[{'label': 'Man', 'value': 'male'},
                        {'label': 'Vrouw', 'value': 'female'}])),

            bulma_field(label='Hoed of pet', component=dcc.Dropdown(id='input-hat', 
                options=[{'label': 'Hoed', 'value': 'hat'},
                        {'label': 'Pet', 'value': 'cap'},
                        {'label': 'Geen', 'value': 'none'}]
                )),

            bulma_field(label='Bril', component=dcc.Dropdown(id='input-glasses', 
                options=[{'label': 'Bril', 'value': 'yes'},
                        {'label': 'Geen Bril', 'value': 'no'}]
                )),

            bulma_field(label='Accessories', component=dcc.Dropdown(id='input-necklace', 
                options=[{'label': 'kleine ketting', 'value': 'necklace'},
                        {'label': 'grote ketting', 'value': 'chain'},
                        {'label': 'geen ketting', 'value': 'none'}]
                        )),

            bulma_field(label='Gezichtsbeharing', component=dcc.Dropdown(id='input-facial_hair', 
    ...
    html.Div([dcc.Input(id='hidden-{}'.format(x), type='hidden', value='') for x in feature_keys]),
#     [
...
def choose_image(dropdown_value):
    ...
    data = main_model.main_model(image_file)
